// Package e2etest provides a multi-VM e2e test suite for the GoJSX framework.
//
// Each VM×renderer×bundler combination registers itself as an AppFixture via
// its fixture file's init(). Tests call forEachApp (or forEachReactApp) to
// run assertions against every registered fixture in a table-driven sub-test.
//
// To add a new VM: create fixture_<vmname>.go, implement AppFixture, and call
// register() from init(). The test files require no changes.
//
// To add a new renderer: create fixture_<vmname>_<renderer>.go with the
// appropriate Renderer() return value. Use forEachReactApp in tests that check
// React-specific Flight protocol output; use forEachApp for renderer-agnostic checks.
package e2etest
...
// SharedBridge returns the blog test bridge used by all e2e fixtures.
