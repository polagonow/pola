...
// promise.js is a moderncquickjs-only polyfill that replaces the native
// Promise with a pure-JS implementation routed through queueMicrotask.
// These tests verify that continuations fire exactly when __drainMicrotasks__
// is called, not via native job scheduling.

import "testing"

func withModernc(t *testing.T, fn func(t *testing.T, f Fixture)) {
	t.Helper()
	t.Run("moderncquickjs", func(t *testing.T) {
		f := newModerncquickjsFixture(t)
...
func TestPromiseResolveThen(t *testing.T) {
	withModernc(t, func(t *testing.T, f Fixture) {
		if err := f.Eval(`
			var result = null;
