// Package promise replaces the native QuickJS Promise with a pure-JS
// implementation that schedules all continuations via queueMicrotask.
//
// modernc.org/quickjs exposes no JS_ExecutePendingJob API, so native
// Promise.then callbacks would never fire. By routing every .then callback
// through our queueMicrotask queue, __drainMicrotasks__() flushes them in the
// render loop.
//
// Must be installed AFTER microtask.Enable (requires queueMicrotask).
package promise
...
// Enable replaces the native QuickJS Promise with a queueMicrotask-backed
// implementation. microtask.Enable must be called first.
