// Package textencoding provides TextEncoder and TextDecoder for the QuickJS VM.
...
// Enable installs TextEncoder and TextDecoder as globals into ctx.
func Enable(ctx *quickjs.Context) error {
	ret := ctx.Eval(src, quickjs.EvalFileName("textencoding.js"))
	defer ret.Free()
	if ret.IsException() {
		return fmt.Errorf("textencoding: %w", ctx.Exception())
	}
	return nil
}
