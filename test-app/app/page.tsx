...
//
// Register Go functions in main.go with inj.Register("name", handler).
// Then call them here with: const result = await di.name(args...)
...
async function Greeting() {
  const { message } = await di.greet("test-app");
    ...
    <div>
      <div style={{ padding: "4rem 0 2rem" }}>
        ...
        <p style={{ color: "var(--color-muted)", fontSize: "1.125rem", marginTop: "0.75rem", maxWidth: 520 }}>
          Your Pola app is running. Edit <code>app/page.tsx</code> and <code>main.go</code> to get started.
        </p>
        <Greeting />
      </div>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--color-muted)", marginBottom: "1rem", fontSize: "0.9375rem" }}>
          The data below is fetched from Go functions registered in <code>main.go</code>,
          called during server-side rendering via <code>{"await di.getServerInfo()"}</code>.
        </p>
        <ServerInfo />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1rem" }}>
        <Card
          title="DI Bridge"
          description="Call Go functions from React using @pola/di. Register services in main.go, call them in any server component."
        />
        <Card
