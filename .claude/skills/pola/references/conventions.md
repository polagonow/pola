...
├── repositories/         data access: <name>_repository.go (interface) + gorm/ (impl)
├── dto/                  request/response DTOs (pola generate dto / scaffold)
├── db/
│   ├── models/           ORM models: gorm/<name>.go  (ent: schema/<name>.go)
│   ├── migrations/       Atlas migrations
│   └── seeds/            seeds.go — Seed(ctx, *core.Registry) (pola generate seed / pola db seed)
...
└── web/
    ...
    ├── mailers/          email templates (pola generate mailer)
    ├── package.json
    └── tsconfig.json
```
...
**API-only apps** (`pola new --api-only` / `api_only = true` in the Polafile) skip the
entire `web/` tree, the bridge, and the JS toolchain — you keep `routes/`, `services/`,
`repositories/`, `dto/`, `db/`. Route structs can implement
`Middleware() []core.RouteMiddleware` (per-route guards, e.g. `auth.RouteMiddleware`) and
`Meta() map[string]any` (OpenAPI summary/tags) — see `references/api.md`.
