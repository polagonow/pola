...
├── repositories/         data access: interface + ORM impl (repositories/gorm/…)
├── dto/                  request/response DTOs (Create/Update use dto.Optional for PATCH)
├── db/
│   ├── models/           ORM models (db/models/gorm/… or db/models/schema/… for ent)
│   ├── migrations/       versioned migrations (Atlas)
│   └── seeds/            seeds.go — run with `pola db seed`
├── mcp/                  optional MCP server: tools/ resources/ prompts/ (pola generate mcp …)
├── public/               static assets / embedded production bundle
└── web/                  the frontend (Next.js App Router style; absent with --api-only)
...
Data-layer chain: **`db/models` → `repositories/` (interface + ORM impl, paginated + filterable) →
`services/` (business logic) → `dto/` (request/response shapes) → `actions/` (React bridge) *or*
`routes/` (HTTP).** See `references/data-layer.md`.

Beyond CRUD, the framework ships production API features — JWT/Basic **auth** +
role **authorization** (`auth`, `auth/authz`), **query filtering** with Goyave-style
operators (`repository.ParseListQuery`), **transactions** (`repository/uow`),
**OpenAPI + SwaggerUI** (`openapi`), **CORS**, **health checks** (`/healthz`,
`/readyz`), rate limiting, flash messages, i18n, and **seed factories**. See
`references/api.md` and `references/data-layer.md`.
...
- **Available blocks:** `csrf`, `security_headers`, `cache`, `database` (+ `migrations`),
  `storage`, `mailer`, `image_processing`, `mcp`, `testing`, `session`, `jwt`, `protect`,
  `rate_limit`, `flash`, `i18n`.
- **Other top-level attributes:** `framework` (HTTP framework: `std` | `gin` | `echo` | `chi`)
  and `api_only = true` (no frontend).
...
|---------|--------------|
| `pola new [name]` | Scaffold a new app (flags: `--renderer --bundler --router --framework --css --vm --ui --pm --module --csrf --security-headers --api-only -y/--yes --pola-path` …) |
| `pola dev` (alias `serve`) | Dev server + hot reload (`-p/--port`, defaults from Polafile/env) |
| `pola build` | Two-stage production build → single binary (`-o`, `--cgo`, `--platform`, `--embed-migrations`, `--migrate`) |
...
| `pola destroy <gen> [args]` | Inverse of generate — removes a generator's files (aliases `d`, `del`; `--dry-run`) |
| `pola db <sub>` | `migrate` · `rollback` · `status` · `reset` · `schema:load` (**sqlite only**) · `seed` |
...
| `repository` | `repo` | interface + ORM impl |
| `service` | `svc` | service struct depending on a repository |
| `dto` | — | `dto/<name>.go`: response + Create + Update DTOs (`dto.Optional` for PATCH) |
...
| `scaffold` | `s` | **model + repository + service + dto + action + route + zod + pages** |
...
| `seed` | — | `db/seeds/seeds.go` — run with `pola db seed` |
| `docs` | — | Fumadocs docs section (Go-native MDX; `content/docs/` + `/docs` routes) |
...
10. **`pola new` prompts unless told not to.** In CI/scripts/agents pass `-y/--yes` (auto-enabled
    when stdin is not a TTY). The `--css` flag default is `none` — pass `--css tailwind` (or a
    `--ui` that implies a CSS processor) explicitly.
...
- **`references/data-layer.md`** — field-spec grammar, the model→repo→service→dto→action/route
  chain with real skeletons, query filtering (`ParseListQuery` + operators), DTOs/`Optional`,
  transactions, seeding, validation, ORM selection, migrations, storage/file uploads.
- **`references/api.md`** — auth (JWT/Basic + `jwt`/`protect` blocks), authorization policies,
  OpenAPI + SwaggerUI, CORS, health checks, rate limiting, flash, i18n, request helpers,
  per-route middleware.
- **`references/recipes.md`** — full worked walkthroughs: blog CRUD via scaffold, an MCP tool over a
  service, file uploads with storage, a filtered REST API with DTOs, JWT-protected APIs, seeding,
  and a Fumadocs docs section.
