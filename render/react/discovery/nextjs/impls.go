...
func PageAlias(p contract.PageEntry) string {
...
func collectSegments(pagesDir, pageDir string) ([]contract.PageSegment, error) {
	absPagesDir, _ := filepath.Abs(pagesDir)
	...
	var segments []contract.PageSegment
	for _, d := range dirs {
		var seg contract.PageSegment
		seg.Dir = d
...
func DiscoverPages(appDir string) ([]contract.PageEntry, error) {
	pagesDir := filepath.Join(appDir, "app")
	var pages []contract.PageEntry
...
// GlobalComponents holds optional global-level components found at app/ root.
...
// global-error.tsx with a default export.
...
func (d *NextJSDiscoverer) Discover(appDir string) (contract.DiscoveryResult, error) {
	...
	clientComponents, err := DiscoverClientComponents(appDir)
	if err != nil {
		return contract.DiscoveryResult{}, err
	}
