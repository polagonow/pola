...
const (
	...
	// BridgeObject is the per-request Go→JS bridge function table object.
	// Created during VM init (empty object) and populated per request by
	// VM.SetBridgeFunctions; read by server components as `__JSI__.fnName(...)`.
	BridgeObject = "__JSI__"
